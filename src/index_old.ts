...
function drawCircle(ctx: CanvasRenderingContext2D
    | OffscreenCanvasRenderingContext2D, circle: Circle): void {
    const fillStyleText = "#" + circle.color.map(n => n.toString(16)).join("")
        + Math.floor(circle.opacity * 255).toString(16);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let lastRenderWidth = renderWidth;
    let renderHeight = window.innerHeight;
    let lastRenderHeight = renderHeight;
    const canvasElt =
        <HTMLCanvasElement>document.getElementById("background-canvas");
    canvasElt.width = renderWidth;
    canvasElt.height = renderHeight;
    const canvasOfs = new OffscreenCanvas(renderWidth, renderHeight);
    const ctx = canvasElt.getContext("bitmaprenderer", { alpha: false })
        ?? (function () { throw new Error("2d context not supported"); })();
    const ctxOfs = canvasOfs.getContext("2d")
        ?? (function () { throw new Error("2d context not supported"); })();
    const obs = new Observer(1, 3, 1e-4, [renderWidth / 2, renderHeight / 2]);
    ...
    function render(): void {
        ...
        if (renderWidth !== lastRenderWidth) {
            canvasElt.width = renderWidth;
            canvasOfs.width = renderWidth;
            lastRenderWidth = renderWidth;
        }
        renderHeight = window.innerHeight;
        if (renderHeight !== lastRenderHeight) {
            canvasElt.height = renderHeight;
            canvasOfs.height = renderHeight;
            lastRenderHeight = renderHeight;
        }

        obs.setWindow([renderWidth / 2, renderHeight / 2]);
        obs.universe.translateY(timeElapsed * 0.02);
        const flattened: Circle[] = obs.flatten();
        ctxOfs.clearRect(0, 0, canvasElt.width, canvasElt.height);
        flattened.forEach(circle => drawCircle(ctxOfs, circle));
